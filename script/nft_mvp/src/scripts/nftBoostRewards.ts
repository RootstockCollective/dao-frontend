...
import { backersManagerAddress, network, rewardCoinbaseAddress } from "./env";
...
async function estimatedGaugeRewards(
  rewardToken: Address,
  backer: Address,
  gauge: Address
): Promise<bigint> {
...
async function main() {
  ...
  if (!nftContractAddress || isNaN(boostPercentage)) {
    throw new Error(
      "Usage: bun run nft-boosted-rewards -- <NFT_CONTRACT_ADDRESS> <BOOST_PERCENTAGE>"
    );
  }
  ...
  const boost = BigInt(noDecimalsFactor * (1 + boostPercentage / 100));
  ...
  for (let i = 0; i < nftTransferEvents.length; i++) {
    ...
    for (const gauge of gauges) {
      const estimatedRBTCRewards = await estimatedGaugeRewards(
        rewardCoinbaseAddress,
        holderAddress,
        gauge
      );
      const estimatedRIFRewards = await estimatedGaugeRewards(
        rewardTokenAddress,
        holderAddress,
        gauge
      );

      const boostedRBTCRewards =
        (estimatedRBTCRewards * boost) / BigInt(noDecimalsFactor);
      const boostedRIFRewards =
        (estimatedRIFRewards * boost) / BigInt(noDecimalsFactor);
