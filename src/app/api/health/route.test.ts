...
describe('GET /api/health/', async () => {
  ...
  it('should pass a health check type', {}, async () => {
    ...
    expect(response.status).toBe(200)
    expect(mockedRunHealthCheck).toHaveBeenCalledOnce()
    expect(mockedRunHealthCheck).toHaveBeenCalledWith('lastBlockNumber')
  })

  it('should pass any args onto the health check', {}, async () => {
    const req = new Request('http://localhost/api/health')
    const response = await GET(req, {
      params: Promise.resolve({ type: 'lastBlockNumber', args: ['foo', 'bar'] }),
    })

    expect(response.status).toBe(200)
    expect(mockedRunHealthCheck).toHaveBeenCalledWith('lastBlockNumber', 'foo', 'bar')
  })
})
