...
export const AllocationBarSegmentVisual = forwardRef<HTMLDivElement, Props>(
  ({ item, totalValue, className, style: additionalStyle, children, onClick }, ref) => {
    // Calculate the percentage width based on the actual value and total value
    const percentageWidth = valueToPercentage(item.value, totalValue)
    ...
    return (
      <div ref={ref} className={cn('h-full', roundedClasses, className)} style={style} onClick={onClick}>
        {children}
      </div>
    )
  },
