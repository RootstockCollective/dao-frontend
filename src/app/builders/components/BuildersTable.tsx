...
const BuildersTable = () => {
  ...
  const showActionsColumnHeader =
    columns.filter(({ hidden }) => !hidden).length === ACTIONS_HEADER_SHOW_THRESHOLD
      ...
      <table>
        <thead>
          <tr className="capitalize text-xs leading-4 bg-foreground">
            ...
            {showActionsColumnHeader && <TableCell>Actions</TableCell>}
          </tr>
        </thead>
        <tbody>
          {builders &&
            Object.entries(builders).map(([address, builder]) => (
              ...
              >
                ...
                {showActionsColumnHeader && <ActionsCell />}
              </tr>
            ))}
        </tbody>
      </table>
