...
export const ConfirmDepositStep = ({ onGoToStep, onCloseModal, setButtonActions }: FlowStepProps) => {
  const { amount, selectedToken, isNative, usdEquivalent } = useSyntheticYieldTopUpContext()
  const amountWei = parseEther(amount)
  const { onRequestTransaction, isRequesting, isTxPending } = useSyntheticYieldTopUp(amountWei, isNative)
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-2 gap-y-10">
      <ConfirmationDetail label="Recipient address" value={shortAddress(syntheticYield.address, 6)} />
      <ConfirmationDetail
