...
export const RequestAllowanceStep = ({ onGoNext, onGoBack, setButtonActions }: FlowStepProps) => {
  const { amount, usdEquivalent, isValidAmount } = useSyntheticYieldTopUpContext()
  ...
  const { isAllowanceEnough, isAllowanceReadLoading, onRequestAllowance, isRequesting, isTxPending } =
    useErc20Allowance(WRBTC_ADDRESS, syntheticYield.address, amount)
  ...
  const handleRequestAllowance = useCallback(() => {
    executeTxFlow({
      ...
      onSuccess: onGoNext,
      action: 'syntheticYieldAllowance',
    })
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-2 gap-y-10 mb-8">
      <div className="flex flex-col gap-2">
        ...
        <Header variant="h1">Synthetic Yield contract</Header>
      </div>
