...
export function useGetProposalsWithGraph() {
  ...
  const activeProposals = useMemo(() => {
    ...
    return proposalsData.proposals
      .filter(
        (proposal: ProposalGraphQLResponse) => handleProposalState(proposal, latestBlockNumber) === 'Active',
      )
      .length.toString()
...
function handleProposalState(proposal: ProposalGraphQLResponse, blockNumber?: bigint): string {
  if (!blockNumber) {
