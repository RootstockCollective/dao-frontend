...
const CopySvg = ({ color = 'white' }) => {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
      <g clip-path="url(#clip0_4759_1391)">
        <path
...
export const Address: React.FC<AddressProps> = ({ address }) => {
  ...
  const feedback = {
    success: { color: '#22AD5C', text: 'Copied!' },
    error: { color: '#FF0000', text: 'Error!' },
  }[copied]

  const copyColor = animationShown ? feedback.color : 'white'
  ...
  const feedbackClass = `text-sm text-[${feedback.color}] animate-fade-in-slide-in`
