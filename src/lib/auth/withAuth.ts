 ...
 */
type AuthenticatedRouteHandler = (request: NextRequest, session: JWTPayload) => Promise<NextResponse>
...
export function withAuth(handler: AuthenticatedRouteHandler) {
  return async (request: NextRequest): Promise<NextResponse> => {
    try {
      ...
      return NextResponse.json({ error: sanitizeError(message) }, { status: 401 })
    }
  }
}
