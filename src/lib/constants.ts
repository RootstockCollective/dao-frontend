...
const FEATURE_FLAGS: Record<Feature, string> = {
  ...
  security_notice: process.env.NEXT_PUBLIC_ENABLE_FEATURE_SECURITY_NOTICE ?? '',
}
...
const DEFAULT_PROPOSAL_METADATA_SYNC_BLOCK_STALENESS_THRESHOLD = 10
/**
 * Max blocks proposal-source metadata (DB `SubgraphMetadata`, The Graph `_meta`) may lag behind
 * the chain head. Separate from {@link STATE_SYNC_BLOCK_STALENESS_THRESHOLD} (indexer health).
 * Override with `NEXT_PUBLIC_PROPOSAL_METADATA_SYNC_BLOCK_STALENESS_THRESHOLD`.
 */
export const PROPOSAL_METADATA_SYNC_BLOCK_STALENESS_THRESHOLD = Number(
  process.env.NEXT_PUBLIC_PROPOSAL_METADATA_SYNC_BLOCK_STALENESS_THRESHOLD ??
    DEFAULT_PROPOSAL_METADATA_SYNC_BLOCK_STALENESS_THRESHOLD,
)
